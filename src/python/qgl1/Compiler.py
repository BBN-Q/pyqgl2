# FIXME: Doesn't work quite right - getting empty sequences
...
from QGL import PatternUtils
from QGL.PatternUtils import flatten
from QGL import Channels
...
def pulses_to_waveforms(physicalWires, verbose=False):
...
def compile_to_hardware(seqs, fileName, suffix='', verbose=False):
    ...
    physWires = pulses_to_waveforms(physWires, verbose)
...
def compile_sequences(seqs, channels=None, verbose=False):
    ...
    for seq in seqs:
        if not isinstance(seq[0], ControlFlow.Wait):
            if verbose: print("Adding a Wait - first seq elem was %s" % str(seq[0]))
...
#        seqs.insert(0, funcDefs)
    ...
    for seq in seqs[1:]:
        wires = compile_sequence(seq, channels, verbose)
...
def compile_sequence(seq, channels=None, verbose=False):
    '''
